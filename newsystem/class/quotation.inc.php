...
Class QUO_REMARKS {
    ...
    function create_remarks() {
        ...
        $quoremtab = 'quotationnew_remarks_' . $com . '_' . $period;
        ...
        $qrdbg = "INSERT INTO $quoremtab SET ";
        $cnt = 0;
...
#echo $qrIns;
    ...
    function extractarray() {
        ...
        $this->set_company(strtoupper($postdata['com'])); //v
...
#echo "</pre>";
    ...
    function extractarray() {
        ...
        $this->set_specialShapeOrder($postdata['specialShapeOrder']);
        $this->set_tabletype($postdata['tabletype']); //v
        $this->set_company(strtoupper($postdata['company'])); //v
...
//extract post data to scope variables.
        ...
        $this->set_specialShapeOrder($quodata['specialShapeOrder']);
...
//List of variables needed in Quotation_$com_$period;
    ...
    private $specialShapeOrder;      //$_POST['specialShapeOrder']
    ...
    private $ftz;           //Free Trade Zone, original Value = NULL
...
#echo $createResult . "<br>";
        ...
        if ($result == 'insert ok!') {
            $info = 'Insert Successful!';
            $qrQID = "SELECT qid FROM $quotab WHERE quono = '$quono' AND item = '$item' ORDER BY qid DESC";
            $objSQLQID = new SQL($qrQID);
            $resultQID = $objSQLQID->getResultOneRowArray();
            $this->set_qid($resultQID['qid']);
        } else {
            $info = 'Insert Failed';
        }
        return $info;
...
//extract post data to scope variables.
        ...
        $this->set_company(strtoupper($postdata['company']));
        ...
        $this->set_odissue('no');
        if (isset($postdata['rev_parent'])) {
            $this->set_rev_parent($postdata['rev_parent']);
        } else {
            $this->set_rev_parent(NULL);
        }
        if (isset($postdata['rev_child'])) {
            $this->set_rev_child($postdata['rev_child']);
        } else {
            $this->set_rev_child(null);
        }
        /*
...
#echo "\$period = $period, \$cid = $cid , \$quotab = $quotab <br>";
        ...
        $sqlQidCheck = "SELECT count(id) FROM $quotab ORDER BY id DESC";
        ...
        if ($recordCount == 0) {
            ...
            $sqlQid = "SELECT id FROM $quotab ORDER BY id DESC";
    ...
    public function quotation_list() {
        ...
        $quotab = $this->quotab;
//        $sqldistinct =
        $sql = "SELECT DISTINCT quono FROM $quotab "
                . "  WHERE cid = $cid AND quono LIKE '%$period%' ";
...
//$sql = "SELECT * FROM $quotab WHERE cid = $cid ORDER BY qid asc ";
        ...
        foreach ($result as $index => $data_row) {
            $qno = $data_row['quono'];
            $sqldet = "SELECT $quotab.* , admin.name FROM $quotab "
                    . "LEFT JOIN admin "
                    . "ON admin.aid = $quotab.aid_quo "
                    . "WHERE cid = $cid AND quono = '$qno' "
                    . "ORDER BY id ASC "
                    . "LIMIT 0,1";
//            echo "sql = $sqldet<br>";
            $objSQLdet = new SQL($sqldet);
            $detresult = $objSQLdet->getResultOneRowArray();
            $result[$index]['company'] = $detresult['company'];
            $result[$index]['cid'] = $detresult['cid'];
            $result[$index]['date'] = $detresult['date'];
            $result[$index]['name'] = $detresult['name'];
            $result[$index]['rev_parent'] = $detresult['rev_parent'];
            $result[$index]['rev_child'] = $detresult['rev_child'];
            $result[$index]['issued_to_quotation'] = $detresult['issued_to_quotation'];
            $result[$index]['odissue'] = $detresult['odissue'];
        }
...
#echo "\$itemtest = $itemtest <br>";
                $moduleResult = (int) $itemtest % 11;
                $remainderResult = $remainder = fmod($itemtest, 11);
